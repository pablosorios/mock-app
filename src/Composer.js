		...
		<div key='composer-screen' className='flex flex-col tablet:bg-foreground tablet:flex-row w-screen h-screen overflow-y-auto print:h-auto'>
			{
				...
				<aside key='total_sidebar'>
					<div key='sidebar' className={`${isHidden ? `${duplicated ? 'top-1 left-4' : 'top-5 left-6'} sideBar` : 'top-0 left-0 sideModal z-20'} group/sidebar`}>
						<div key='sidebar-container' className={`${isHidden ? 'flex-row tablet:flex-col desktop:flex-row justify-center desktop:justify-start' : 'flex-row-reverse justify-start'} z-20 flex items-center w-auto p-1`}>
							<button key='sidebar-toggle' className={`${isHidden ? 'tablet:hidden' : 'desktop:hidden'} flex justify-center items-center fill-text mr-3 h-6 w-6 z-20`} onClick={toggleSidebar} onMouseOver={toggleSidebar}>
						...
						{children.map((item, index) => (
							<Menu key={`menu-component[${index}]`} route={`menu[${index}]`} space={item_name} menu={subitem} submenu={submenu} item={item} translate={translate} language={language} isHidden={isHidden} toggleSidebar={toggleSidebar}/>
						))}
					</div>
			...
			<div key='content-container' className={`${key_count > 0 && duplicated ? 'flex-col' : 'flex-col-reverse'} w-full h-screen flex tablet:flex-col print:h-auto`}>
				<nav key='navbar' className='w-full h-auto bg-foreground flex flex-row desktop:pt-1.5 desktop:pb-1'>
					{keys.map((key, index) => (
					...
					<div key='profile' className='flex justify-end items-center tablet:grow'>
						<div key='language-box' className={`${!isHidden ? 'z-0' : 'z-10 tablet:z-0'} bg-foreground shadow-lg shadow-shadow/50 border border-border tablet:border-none tablet:shadow-none tablet:mt-0 rounded-xl tablet:rounded-none tablet:bg-transparent justify-center items-center px-1 fixed top-5 right-4 py-1 flex flex-row tablet:static tablet:flex-col desktop:flex-row tablet:pt-1 desktop:p-1`}>
...
function Company({isHidden, company}) {
	return (
		<div key='company' className={`${isHidden ? 'company tablet:mb-2 tablet:mt-0.5' : 'companyModal my-2 tablet:my-1'}  desktop:my-1 z-20`}>
			<svg key='company-svg' version='1.1' xmlns='http://www.w3.org/2000/svg' className={`${isHidden ? ' tablet:w-7 tablet:h-7 tablet:mt-1' : ''} rounded-lg w-8 h-8 desktop:w-8 desktop:h-8 desktop:mt-0 bg-icon`} viewBox='40 40 432 432'>
...
function Space({route, item, current, name, translate, language}) {
	...
	return(
		<Link key={`${route}-box`} {...current ? null : {to: name === 'cv' ? '/cv': '/dashboard/' + name}} className={`${current ? 'fill-primary text-primary' : 'hover:bg-primary hover:fill-white hover:rounded-xl hover:text-white fill-text text-text'} my-1 mx-auto tablet:mx-2 group bg-transparent w-auto min-w-max rounded-md bg-foreground text-xs font-bold transition-all duration-100 scale-100 origin-bottom desktop:group-hover:text-white desktop:group-hover:bg-transparent py-1 tablet:pt-0 tablet:pb-2 desktop:py-0 flex flex-col desktop:flex-row items-center`}>
...
function Menu({ route, space, menu, submenu, item, translate, language, isHidden, toggleSidebar }) {
	...
	return (
		<div key='menu-differentiator' {...(isHidden && !isOpen && item.hasOwnProperty('children')) ? { onMouseOver: () => { toggleSidebar(); toggleSubMenu()}, onClick: () => { toggleSidebar(); toggleSubMenu()} } : null} {...(!isHidden && !isOpen) ? { onMouseOver: toggleSubMenu, onClick: toggleSubMenu } : null} {...isOpen ? { onMouseLeave: toggleSubMenu, onClick: toggleSubMenu} : null} className={`w-full group/sideMenu flex flex-col pl-2 tablet:pl-1 desktop:pl-2`}>
			{current || item.hasOwnProperty('children') ?
				<div key={`${route}-container`} className={`${isOpen ? 'bg-background rounded-l-lg' : ''}`}>
					<div key={`${route}-box`} {...isOpen ? {onClick: toggleSubMenu} : null} className={`${isOpen ? (item.hasOwnProperty('children') ? 'rounded-l-lg bg-primary fill-white text-white py-1 my-0' : '') : (isHidden ? 'py-1 tablet:pt-0 tablet:pb-1 desktop:py-1 my-1 tablet:my-1.5 desktop:my-1': 'py-1 tablet:py-3 my-1 tablet:my-1 desktop:py-1')} ${isHidden ? 'hidden tablet:flex tablet:sideMenu desktop:sideMenuModal' : 'flex sideMenuModal'} ${current ? 'bg-background fill-primary text-primary' : 'bg-foreground fill-text text-text'} z-20 relative`}>
						...
						<svg key={`${route}-arrow`} className={`${item.hasOwnProperty('children') ? 'scale-100' : 'scale-0'} ${isHidden ? 'tablet:absolute desktop:static tablet:left-1 tablet:top-1.5' : 'static'} ${isOpen ? 'rotate-0' : 'rotate-180'} bg-transparent shadow-none py-1 h-7 w-7 z-10`} xlms='http://www.w3.org/2000/svg' viewBox='0 -960 960 960' >
					...
					{item.hasOwnProperty('children') && isOpen && !isHidden ? 
						children.map((item, index) => (
							<Submenu key={`submenu-component[${index}]`} route={`${route}[${index}]`} current={current} space={space} menu={item_key} submenu={child_route} item={item} translate={translate} language={language} isHidden={isHidden} toggleSidebar={toggleSidebar} toggleSubMenu={toggleSubMenu}/>
						)) : null
					}
				</div>
				:
...
function Submenu({route, current, space, menu, submenu, item, translate, language, isHidden, toggleSidebar, toggleSubMenu}) {
	const icon = item.icon
